...
void Lab4::RenderScene() {
    ...
    modelMatrix = glm::mat4(1);
    modelMatrix *= transform3D::Translate(10 + tx, 1.5f, 10 + tz);
    modelMatrix *= transform3D::RotateOY(animRot);
    ...
    modelMatrix *= transform3D::Translate(10 + tx, 1.5f, 10 + tz);
    modelMatrix *= transform3D::RotateOY(animRot);
    modelMatrix *= transform3D::Translate(4, 0, 4);
    modelMatrix *= transform3D::RotateOY(animRot);
    RenderMesh(meshes["box"], shaders["VertexNormal"], modelMatrix);
...
void Lab4::Update(float deltaTimeSeconds)
{
    ...
    if (animRot >= 360)
        animRot -= 360;
    animRot += speed * deltaTimeSeconds;

    tx += deltaTimeSeconds / 3;
    tz += deltaTimeSeconds / 3;
}
...
void Lab4::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (window->KeyHold(GLFW_KEY_I))
        miniViewportArea.y += speed * deltaTime * 100;

    if (window->KeyHold(GLFW_KEY_K))
        miniViewportArea.y -= speed * deltaTime * 100;

    if (window->KeyHold(GLFW_KEY_J))
        miniViewportArea.x -= speed * deltaTime * 100;

    if (window->KeyHold(GLFW_KEY_L))
        miniViewportArea.x += speed * deltaTime * 100;

    if (window->KeyHold(GLFW_KEY_U)) {
        miniViewportArea.height -= speed * 100 * deltaTime;
        miniViewportArea.width -= speed * 100 * deltaTime;
    }
